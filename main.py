...
async def _send_deadline_for_team(db, team_id: int, team_name: str, *, force: bool = False,
    ...
    # 보고받는 사람(팀장 teams.leader_name / 그룹장 divisions.head_name)은 마감 제외 → 독촉 알림도 안 감.
    # (팀장은 대개 is_report_target=False 로도 빠지지만, 플래그가 잘못 설정돼도 새지 않도록 이름으로 이중 방어)
    result = await db.execute(text("""
        ...
        WHERE m.team_id = :tid
          ...
          AND m.name NOT IN (SELECT head_name FROM divisions WHERE COALESCE(head_name, '') <> '')
          AND r.id IS NULL
