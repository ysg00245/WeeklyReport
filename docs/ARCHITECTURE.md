...
미제출자에게 마감 전 푸시를 보냅니다. 시각은 팀별 설정(`deadline_config.notify_times`)입니다.

```
서버 기동 / 마감설정 저장
      ↓
refresh_deadline_jobs()          ← main.py
      ↓ 전 팀 설정에서 알림 시각(HH:MM)의 합집합을 수집
그 시각마다 cron 잡 1개씩 등록    ← 예: 09:00, 12:00 두 개
      ↓ 해당 시각에 발화
각 팀을 순회하며 판단:
   "마감일 + day_offset == 오늘" 이고 "시각 일치" 인가?
      ↓ 그렇다면
그 팀의 미제출자에게만 Web Push
```

`notify_times` 는 마감일 기준 **상대 지정**입니다.
...
**왜 이렇게 했나**: 예전에는 매일 정오에 한 번 돌면서 "마감까지 0~36시간이면 발송"하는
방식이었습니다. 마감 시각을 바꾸면 알림 시점이 함께 어긋났고, 마감을 정오로 옮기자
당일 알림이 발송 범위 밖으로 빠져 **조용히 사라지는** 문제가 있었습니다.

⚠️ **매시간 폴링으로 바꾸지 마세요.** 서버리스 DB의 자동 절전이 깨져 요금이 급증합니다.
하루 DB 접근 횟수는 "서로 다른 알림 시각의 수"로 제한됩니다.
