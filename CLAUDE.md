...
마감 알림 스케줄러도 같은 이유로 **매시간 폴링하지 않습니다.** 전 팀 설정에서
알림 시각의 합집합을 모아 그 시각에만 cron 을 등록합니다(`main.py:refresh_deadline_jobs`).
"조건에 맞는지 자주 확인하자"는 방향으로 바꾸지 마세요.
...
- [`docs/AI_SUMMARY.md`](docs/AI_SUMMARY.md) — AI 요약: 모델·비용·프롬프트 구조 (알려진 제약 포함)
