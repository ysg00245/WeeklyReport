...
# 프로젝트별 — 세로형: 표를 쓰지 않고 '이번주/차주/이슈' 를 세로로 쌓는다.
# 표는 셀 폭에 눌려 긴 내용이 읽기 어렵고 모바일에서 특히 불리하다. 내용이 많은 팀에 적합.
DEFAULT_FORMAT_PROJECT_VERTICAL = """[과제]
위 팀원들의 주간보고를 **프로젝트 단위**로 정리하여, 팀장이 열자마자 상황을 파악할 수 있는
**「한눈 요약 → 이슈 라인 → 프로젝트별 세로 목록」** 구조의 마크다운 요약을 작성합니다.
(표를 쓰지 않는다. 이번주/차주/이슈를 **위에서 아래로 쌓는** 형태다.)

[전체 구조 — 위에서 아래로 정확히 이 순서]
1. **한눈 요약** (blockquote 한 개): 팀 전체 흐름을 1~2문장으로 압축.
2. **이슈 라인** (있을 때만): 팀장이 알아야 할 이슈를 ⚠️ 한 줄씩.
3. **프로젝트별 섹션**: `### 프로젝트명 (N명)` 헤더 + `**이번주** / **차주** / **이슈 및 특이사항**` 소제목과 bullet 목록.
4. **미제출자 라인** (있을 때만).

[프로젝트 그룹핑 — 입력 그룹을 그대로 따를 것 (매우 중요)]
- 입력 데이터는 이미 `### [그룹: {프로젝트명}]  인원 N명` 형태로 **그룹이 나뉘어** 들어온다. **이 그룹 구조를 그대로 따른다.**
- 입력의 각 `### [그룹: X]` → 출력의 `### X ({N}명)` 섹션 **하나**로. 그룹을 **임의로 합치거나 나누거나 이름을 바꾸지 말 것.**
  - 예: 어떤 팀원의 보고 본문에 다른 프로젝트명이 나와도, 그 팀원이 입력상 `프로젝트A` 그룹이면 **프로젝트A 섹션 안에** 둘 것.
- 🔴 **완전성 필수**: 입력의 **모든 그룹**과 각 그룹의 **모든 팀원**이 출력에 반드시 포함. **단 한 명, 단 한 그룹도 누락 금지.** 특히 인원 1명짜리 그룹(기타·운영 등)을 빠뜨리기 쉬우니 출력 전 입력 그룹 수와 출력 섹션 수가 같은지 확인.
- 🔴 **날조 금지**: 입력 팀원 목록에 **없는 사람**(보고 본문에 언급된 외부 인물·타 부서·협력사 인원 등)을 팀원이나 담당자로 만들지 말 것.
- 같은 그룹 내 세부 영역이 나뉘면(예: 로그인/결제/알림) bullet 앞에 `로그인 — ` 처럼 영역 라벨을 붙여 구분.
- 섹션 순서는 입력 그룹 순서를 유지.

[세로 목록 작성 규칙 — 매우 중요]
- 각 프로젝트 섹션 안에 **`**이번주**` / `**차주**` / `**이슈 및 특이사항**`** 세 소제목을 이 순서로 둔다.
- 각 소제목 아래는 **`- ` 로 시작하는 마크다운 bullet 목록**. 표(`|`)를 쓰지 말 것.
- 🔴 **`<br>` 를 쓰지 말 것.** 표가 아니므로 항목마다 자연스럽게 줄을 나눈다(각 bullet 이 한 줄).
- 각 bullet 끝에 **담당자 괄호** 표기: `(홍길동 과장)`. 🔴 담당자는 **그 그룹의 실제 입력 팀원 이름만.**
- 한 bullet 이 길면 쪼개지 말고 한 줄로 쓴다(세로형은 폭 제약이 없다). 다만 서로 다른 업무는 각각의 bullet 으로.
- 소제목당 bullet 은 3~6개로 **압축**. 분량이 매우 많으면 화면/모듈 단위로 묶고 세부는 대표 2~3개 + "…등".
- **운영(정기 작업) 보고**: 날짜별 나열을 그대로 옮기지 말고 **업무 단위로 묶어** 날짜는 괄호로.
  - ❌ `- 7/15 배치 검증` / `- 7/15 등록 해제` / `- 7/16 등록 확인` (날짜 나열)
  - ✅ `- 정기 배치 — 검증·발송 컨펌(7/15), 외부 연동 자료 전송 2건(7/16) (윤서연 사원)`
- **이슈 및 특이사항에 내용이 없으면 그 소제목 자체를 생략**한다. (표와 달리 빈 칸을 채울 필요가 없다. `-` 만 남기지 말 것)
- 평가형 문구("전체적으로 잘", "성공적으로") 금지. 사실 기반만. 명사형 종결("~완료", "~진행"). 프로젝트ROLE 은 출력 금지.

[한눈 요약(blockquote) 규칙]
- `> **이번 주 한눈에**: ...` 형식의 물리적 한 줄. 내용은 2~3문장.
- 🔴 **문장(호흡) 단위로 `<br>` 개행** — 여기는 blockquote 라 `<br>` 를 쓴다(본문 목록과 다름).
- 팀이 이번 주 만들어낸 것의 큰 그림 + 다음 주 방향. 개인 나열 금지, 프로젝트/흐름 단위로.

[이슈 라인 규칙 — 엄격]
- 형식: `⚠️ **{이슈 요지}** — {핵심 내용·필요 조치} ({이름} {직급})`
- ⚠️ 에는 **팀장의 조율·의사결정이 필요하거나 일정을 위협하는 리스크만** 올린다.
- 🔴 **정상적으로 진행 중인 업무는 이슈가 아니다.** 정상 진행 항목을 ⚠️ 로 올리거나 "미완료"라고 표현하지 말 것. → 해당 프로젝트의 "이슈 및 특이사항" 목록에 `~진행 중(진척 N%)` 식으로만 둔다.
- 단순 휴가/반차도 ⚠️ 아님.
- 올릴 리스크가 하나도 없으면 이 블록 자체를 생략.

[운영(ops) 수치 항목 처리]
- 운영 카운터(접수/처리/변경 등) 건수는 `운영` 섹션 **이번주 첫 bullet**: `- 접수 3건 / 처리 1건 / 변경 2건` (실제 라벨은 입력에 온 그대로).
- 0건 항목 생략, 모두 0건이면 라인 생략. 운영 인원 여럿이면 합산.
...
(프로젝트 수만큼 섹션 반복. 이슈가 없는 프로젝트는 '이슈 및 특이사항' 소제목 생략)
...
[출력 시 주의]
- blockquote(`>`) 는 맨 위 **한 개만**. 코드블록 금지.
- 🔴 **표를 만들지 말 것.** `|` 문자로 시작하는 줄이 하나라도 있으면 잘못된 출력이다.
- 🔴 **본문 목록에 `<br>` 금지.** (blockquote 한눈 요약에서만 허용)
- 이슈 라인(⚠️)에 올린 이슈도 해당 프로젝트의 이슈 목록에 **중복 기재**.
- 🔴 **출력 직전 자가 점검 3가지**: (1) 입력의 **모든 그룹**이 `### 그룹 (N명)` 섹션으로 있는가? **운영 그룹도 반드시 별도 섹션으로.** (2) 담당자 괄호에 입력 팀원이 **아닌** 이름은 없는가? (3) `|` 로 시작하는 표 줄이 없는가?
"""
...
async def summarize_all(
    ...
    if mode == "team":
        ...
        # 하위 형식 3종: 섹션형(project, 기본) / 단일표(project_flat) / 세로형(project_vertical)
        if req.summary_type == "project_flat":
            task_prompt = await _load_prompt(db, "ai_format_project_flat", DEFAULT_FORMAT_PROJECT_FLAT, tid)
        elif req.summary_type == "project_vertical":
            task_prompt = await _load_prompt(db, "ai_format_project_vertical", DEFAULT_FORMAT_PROJECT_VERTICAL, tid)
        else:
...
async def get_prompts(
    ...
    defaults = {
        ...
        "ai_format_project_vertical": DEFAULT_FORMAT_PROJECT_VERTICAL,
